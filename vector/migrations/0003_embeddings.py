"""ChromaCollection rows embed their name + description (pgvector) for the semantic ``search`` filter.
...
def backfill_embeddings(apps: Any, schema_editor: Any) -> None:
    """Embed every ChromaCollection that has text, with the configured model; no-op when disabled."""
    from embeddings import engine

    if not engine.enabled():
        return
    model = apps.get_model("vector", "ChromaCollection")
    current = engine.model_id()
    queryset = model._default_manager.exclude(embedding_model=current).order_by("pk").only("pk", "name", "description")
    while True:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ...
        ("llm", "0004_alter_usagerecord_status"),
        ("vector", "0002_alter_chromacollection_options_and_more"),
