...
async def _prepare(documents: List[inputs.DocumentInput], embedder: LLMModel) -> Dict[str, Any]:
    ...
    embeddings = await aembed_texts(embedder, contents)
...
async def add_documents_to_collection(info: Info, input: inputs.AddDocumentsToCollectionInput) -> List[types.Document]:
    ...
    payload = await _prepare(input.documents, collection.embedder)
