...
from llm.logic import detect_features
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            help='Ollama API URL (default: from settings.OLLAMA_URL)'
        )
        parser.add_argument(
            '--name',
            type=str,
        ...
        parser.add_argument(
            '--force',
            action='store_true',
            help='Force recreation of provider if it already exists'
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if response.status_code != 200:
                self.stdout.write(
                    self.style.ERROR(f"Failed to connect to Ollama at {ollama_url}. Status: {response.status_code}")
                )
                return
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Successfully connected to Ollama. Found {len(models)} models.")
            )
        ...
        try:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"Created new provider: {provider.name}")
                )
            else:
            ...
            self.stdout.write(
                self.style.ERROR(f"Failed to create provider: {str(e)}")
            )
            return
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Successfully refreshed {len(new_models)} models for provider {provider.name}")
            )
            ...
            self.stdout.write(
                self.style.ERROR(f"Failed to refresh models: {str(e)}")
            )
            return
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(f"Could not get additional Ollama info: {str(e)}")
            )

        self.stdout.write(
            self.style.SUCCESS("Ollama inspection completed successfully!")
        )
