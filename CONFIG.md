# Alpaka — Configuration Reference

This document explains how the **alpaka** service is configured, then lists every
...
[`alpaka_server/configuration.py`](alpaka_server/configuration.py); this file documents
it for humans. If the two ever disagree, the code wins — and you can always print the
live, resolved configuration with `python manage.py validate_settings` (see below).
...
```bash
ARKITEKT_CONFIG_FILE=/etc/alpaka/config.yaml python manage.py runserver
```
...
### LLM / vector-store settings (top-level)

Alpaka adds a handful of top-level (un-nested) fields that configure its LLM providers
and vector store. These live at the root of the config — not inside a block — so their
env vars have no `__` prefix.

| Key | Env var | Type | Default | Description |
|---|---|---|---|---|
| `provider_partners` | — (use YAML) | list[object] | `[]` | Pre-declared LLM providers; those with `auto_configure` are provisioned for every new organization. |
| `ollama_url` | `OLLAMA_URL` | str | `http://ollama:11434` | Base URL of the Ollama server. |
| `chroma_db_host` | `CHROMA_DB_HOST` | str | `chromadb` | ChromaDB vector store host. |
| `chroma_db_port` | `CHROMA_DB_PORT` | int | `8000` | ChromaDB vector store port. |

`provider_partners` mirrors lok's `kommunity_partners`: each entry is a pre-declared LLM
provider. Run `python manage.py ensurepartners` to load them into the DB as `ProviderPartner`
rows. Any partner with `auto_configure: true` is materialized into a real per-organization
`Provider` automatically whenever an organization is created.

Each entry supports: `name`, `identifier` (unique), `kind`, `description`, `short_description`,
`logo_url`, `api_key`, `api_base`, `additional_config`, `auto_configure`, and an optional
`filter_config` (`email_domain_equals` / `email_domain_ends_with` / `username_equals` /
`username_contains`). Prefer the YAML file for credentials.
...
Rooms (title + description) and Chroma collections (name + description — the collection's
metadata, not its documents, which Chroma embeds with the collection's own LLM embedder) embed that text into a pgvector
...
The `search` argument of `rooms` and `chromaCollections` then matches a substring of the
title/name **or** a description whose
...
(`alpaka_server/asgi.py` starts it; `embeddings/healer.py` is the loop) in row-locked batches —
...
migration that first nulls the columns (`UPDATE kammer_room SET embedding = NULL,
embedding_model = ''`, likewise `vector_chromacollection` — Postgres refuses
